...
<div class="main-content">
        ...
        <!-- FLASH MESSAGES -->
                          ...
                          <div class="box">
                            <div class="box-header">
                              ...
                              <ul class="nav nav-tabs nav-tabs-left">
                                <li class="active"> <a href="#list" data-toggle="tab"><i class="icon-align-justify"></i> permissions list </a></li>
                                      ...
                                      <tbody aria-relevant="all" aria-live="polite" role="alert">
                                        ...
                                        if(!is_null($all_permissions)) {
                                          ...
                                          foreach ($all_permissions as $al_permissions) {
                                            ...
                                            ?><td class=" " align="center"><a href="editPermissions?key=<?php echo $al_permissions->permission_id;?>" rel="tooltip" data-placement="top" data-original-title="edit" class="btn btn-blue"> <i class="icon-wrench"></i> </a> <a href="deletePermission?key=<?php echo $al_permissions->permission_id;?>" onclick="return confirm('Are you sure you want to delete this record?')" rel="tooltip" data-placement="top" data-original-title="delete" class="btn btn-red"> <i class="icon-trash"></i> </a></td>
                                          </tr>
                                          <?php
                                          $i++;
                                          }
                        ...
                        <?php
                            if ($this->session->userdata('add_perm')) {
                              if ($this->session->userdata('add_perm') == 'y') {
                                $msg = 'successful! permission has been added.';
                                echo '<script type="text/javascript">alert("'.$msg.'"); </script>';
                              } else {
                                $msg = 'sorry! could not add permission. please try again later';
                                echo '<script type="text/javascript">alert("'.$msg.'"); </script>';
                              }
                              $this->session->unset_userdata('add_perm');
                            }

                            if ($this->session->userdata('ed_perm')) {
                              if ($this->session->userdata('ed_perm') == 'y') {
                                $msg = 'successful! permission has been edited.';
                                echo '<script type="text/javascript">alert("'.$msg.'"); </script>';
                              } else {
                                $msg = 'sorry! could not edit permission. please try again later';
                                echo '<script type="text/javascript">alert("'.$msg.'"); </script>';
                              }
                              $this->session->unset_userdata('ed_perm');
                            }

                            if ($this->session->userdata('del_perm')) {
                              if ($this->session->userdata('del_perm') == 'y') {
                                $msg = 'successful! permission has been deleted.';
                                echo '<script type="text/javascript">alert("'.$msg.'"); </script>';
                              } else {
                                $msg = 'sorry! could not deleted permission. please try again later';
                                echo '<script type="text/javascript">alert("'.$msg.'"); </script>';
                              }
                              $this->session->unset_userdata('del_perm');
                            }
